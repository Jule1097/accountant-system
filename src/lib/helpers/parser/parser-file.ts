...
export function ensureParserFileSize(fileName: string, mimeType: string, fileSize: number): void {
  ...
  if (isParserPdfMimeType(mimeType)) throw new ApplicationError(applicationErrorCodes.validation, `El archivo ${fileName} excede el l\u00edmite de 2MB para PDFs.`, "Parser PDF file size validation failed")
  if (isParserImageMimeType(mimeType)) throw new ApplicationError(applicationErrorCodes.validation, `El archivo ${fileName} excede el l\u00edmite de 4MB para im\u00e1genes.`, "Parser image file size validation failed")
  throw new ApplicationError(applicationErrorCodes.validation, `El archivo ${fileName} tiene un tipo no soportado.`, "Parser file type validation failed")
}
...
export async function toParserAcceptedFile(file: File): Promise<ParserAcceptedFile> {
  ...
  return { fileName: file.name, mimeType, fileSize: file.size, buffer, fileHash: buildParserFileHash(buffer) }
}
...
export async function collectParserAcceptedFiles(formData: FormData): Promise<ParserAcceptedFile[]> {
  ...
  for (const fileEntry of fileEntries) acceptedFiles.push(await toParserAcceptedFile(fileEntry))
